### Actual Evapotranspiration data set manipulation for the analysis ###
  ...
  aet_dat <- read_rds(file.path(CLIMATE.LOC, "extraction_051925.RDS"))
  
  aet_dat <- aet_dat %>% filter(year > 1980, var == "aet") 
  
# Use the tree dataset from Data_Prep_Analysis.R to get the years needed for each plot
data_aet_yrs <- tree.plt.data %>% 
...
#ggplot(data_aet_yrs, aes(REMPER)) + geom_histogram()
# plots:
#n.plt <- nrow(data_aet_yrs)
# Plots between 9 & 11 yrs
#n.9.11 <- nrow(data_aet_yrs %>% filter(REMPER >= 9 & REMPER <= 11))
#n.9.11/n.plt
# 95% of plots have REMPER between 9 and 11 yrs

#### Check on missing aet data -----------------------
#sub_aet <- aet_dat %>% dplyr::select(puid, var) %>% distinct()
#data_aet_check <- left_join(data_aet_yrs, sub_aet, by = "puid")  %>%
#  mutate(state = substr(puid, 7, 8),
#         state = as.numeric(gsub("_", "", state)))
#nrow(data_aet_check)      #18207 plots
#missing.aet <- data_aet_check %>% filter(is.na(var)) # 3690 rows
#  Missing about 20% of plot aet values

#data_aet_check2 <- data_aet_check %>% tibble() %>% filter(is.na(var))


#### Analyzing climate variable  data  ------------------------------------------------

  var1 <- aet_dat
  data_yrs <- data_aet_yrs
