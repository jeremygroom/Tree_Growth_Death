...
mort.grow.dat <- readr::read_rds(unzip(paste0(DATA.LOC, ANALYSIS.TYPE, "_dat_", var.filename, ".zip"), paste0(DATA.LOC, ANALYSIS.TYPE, "_dat_", var.filename, ".rds")))

# Deleting written RDS file (~150 MB)
if (file.exists(paste0(DATA.LOC, ANALYSIS.TYPE, "_dat_" , var.filename, ".rds")) == TRUE) {
  file.remove(paste0(DATA.LOC, ANALYSIS.TYPE, "_dat_" , var.filename, ".rds"))
}
