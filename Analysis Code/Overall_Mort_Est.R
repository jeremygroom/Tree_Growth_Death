...
t.mort.all <- map(as.numeric(gsub("X", "", SEL.SPP)), clim.mort.resp.fcn, clim.var = CLIM.VAR.USE, treedat.sel = treedat.use, clim.dat = climate.use)
a.mort.all <- parse.tree.clim.fcn(t.mort.all, clim.var = CLIM.VAR.USE, analysis.type = "mort", resp.dat = "died.out", tot.dat = "all.trees", selected.spp = SEL.SPP, clim.dat = clim.dat)
      ...
      # State-level mortality estimates.
        ...
        state.ests2 <- rbindlist(state.ests) %>% data.frame() %>%  # data.table function
          mutate(State = case_match(Quantile, 1 ~ "CA", 2 ~ "WA", 3 ~ "OR")) %>%
          select(-Quantile)
        
          # Writing state values to file
        fwrite(state.ests2, file = paste0(RESULTS1.LOC, "Mort_figs_", CLIM.VAR.USE, "/State_Mort_Ests_", CLIM.VAR.USE, ".csv")) 
      ...
      # All states combined mortality
        ...
        fwrite(all.mort.ests, file = paste0(RESULTS1.LOC, "Mort_figs_", CLIM.VAR.USE, "/All_Mort_Ests_", CLIM.VAR.USE, ".csv")) 
