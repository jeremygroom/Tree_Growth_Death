---
title: "Supplemental material for: Tree growth, mortality, and climatic water deficit in west-coast states, USA"
date: "`r Sys.Date()`"
...
# Growth data by DBH for spp overall, states, differences
grow.dbh.state <- read_csv(paste0(RESULTS1.LOC, "Growth_figs_", CLIM.VAR.USE, "/State_DBH_cm_", CLIM.VAR.USE, ".csv"))
grow.dbh.all <- read_csv(paste0(RESULTS1.LOC, "Growth_figs_", CLIM.VAR.USE, "/All_DBH_cm_", CLIM.VAR.USE, ".csv"))
grow.dbh.diff <- read_rds(paste0(save.loc.fcn(3), "Processed_6Domain_Data.RDS"))
...
# State growth table, DBH (cm)
# Species scientific names
spp.names.use <- analysis.stats$gym.angio.dat

s.g.d.dat <- state.table.fcn(grow.dbh.state, grow.dbh.all, spp.cat, 2)
growth.dbh.ft <- grow.mort.ft.fcn(s.g.d.dat)



ft.lm1 <- CWD.rels$ft_lm1

cwd.init2 <- cwd.init %>% dplyr::select(-UCI, -LCI)
cwd.diff2 <- cwd.diff %>% dplyr::select(-UCI, -LCI)
...
tab.tbl_dbhgrowth <- tab_nums(name = "tbl_dbhgrowth", 
                        caption = "Decadal diameter growth estimates by state and overall for analysis species (cm/decade). Mean estimates with confidence intervals and plot number are provided. Estimates are not made for states with fewer than 10 plots. Ecological category abbreviations (Category) are provided in the text.")
...
fig.dbhdiff_grow <- fig_nums(name = "fig_dbhdiffgrow", 
                          caption = paste0("Differences in mean diameter growth rates (cm/decade) between domains by species.  Domain abbreviations are described in the legend for Figure ",
                                           f.ref("fig_domains"), 
...
fig.negcwdmap_cap <- fig_nums(name = "fig_cwdnegmap", 
                        caption = "Public FIA plot locations for plots with negative CWD values.  The initial CWD values for those plots are displayed in A while the negative delta CWD values are displayed in B.  The inset map (B) shows the locations of the study states within the USA: From north to south, Washington, Oregon, and California.")
...
```{r tbl-dbhgrowth, tab.cap=tab.tbl_dbhgrowth}
...
```{r diff_dbhgrow_fig, fig.height = 6.5, width = 6.5, fig.dpi=300, fig.cap=fig.dbhdiff_grow, fig.cap.sep=""}

# Output from Manuscript_information.R
knitr::include_graphics(paste0(RESULTS1.LOC, "/DBH_Growth_figs_cwd/DBHGrowth_Panel_Plot.jpeg"))
...
```{r cwdfig-negcwdmap, fig.height = 7,  fig.dpi=300, fig.cap=fig.negcwdmap_cap, fig.cap.sep=""}

# Output from Manuscript_information.R
knitr::include_graphics(paste0(RESULTS.OTHER, "cwdneg_maps_figure.png"))
